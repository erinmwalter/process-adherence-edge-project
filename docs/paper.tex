...
I propose an edge-deployed computer vision system running on an NVIDIA Jetson Nano that:
...
\begin{enumerate}
    ...
    \item Transmits only structured cycle results (${\sim}500$\,bytes) over MQTT \cite{b7} to a PC-based dashboard for historical analytics, reducing network usage by over 161{,}000$\times$ compared to streaming compressed video.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item YOLOv8n-pose inference \cite{b1}---produces 17 COCO keypoints \cite{b6} per detected person. I extract keypoints 9 (left wrist) and 10 (right wrist).
...
\textbf{Bandwidth:} Transmitting ${\sim}500$\,bytes per cycle instead of streaming 640$\times$480@30fps video (measured at 1.34\,MB/s at JPEG quality 80) reduces network usage by over 161{,}000$\times$, dropping from ${\sim}4.8$\,GB/hour to ${\sim}30$\,KB/hour per workstation.
...
\textbf{Challenge 1: False zone entries from hand jitter.} When an operator's wrist moves near the boundary of a zone, frame-to-frame noise in the pose estimation could cause rapid zone enter/exit flickering. I addressed this with a debounce mechanism: a zone entry is only registered after the wrist remains inside the zone for 5 consecutive frames (${\sim}167$\,ms at 30\,fps). This eliminates false triggers while keeping the detection responsive enough for normal pick speeds.

\textbf{Challenge 2: Supporting multiple trim levels dynamically.} Different vehicles on the same assembly line require different pick sequences. I designed a two-part solution: (1) a JSON-based trim configuration file that maps trim levels to expected zone sequences, and (2) MQTT-based trim assignment where the line system publishes the trim level for the next vehicle, and the edge device automatically starts a new cycle with the correct expected sequence.

\textbf{Challenge 3: Zone configuration without hardcoding.} Bin positions change between workstations and can shift over time. I built an interactive zone configuration GUI that overlays on the live camera feed, allowing supervisors to click-draw polygon zones and assign names and types. Zones are persisted to JSON and loaded at runtime, making the system portable across stations with zero code changes.

\textbf{Challenge 4: Operating with or without network connectivity.} The edge device must provide operator alerts even when the network is down. I implemented a dummy MQTT client that silently replaces the real MQTT client when no broker is available. The full detection-and-alert pipeline runs locally; network connectivity only affects whether cycle results are forwarded to the dashboard.

% ================================================================
\section{Evaluation}

\subsection{Experiment Setup}
I evaluated the system along four dimensions: inference latency, end-to-end feedback latency, resource utilization, and network bandwidth consumption.

To produce a fair comparison between edge and centralized processing, I built a \textit{benchmark mode} that measures both paths on identical captured frames. Fig.~\ref{fig:benchmark_arch} illustrates the two measurement paths:
...
\begin{itemize}
    ...
    \item PC: Apple MacBook Pro (M3 Pro, 12-core, 36\,GB RAM), macOS
    \item Network: Local Ethernet connection between Jetson and PC
    \item MQTT broker: Mosquitto 2.0 running on the PC
    \item Test scenario: 30 cycles across 3 trim levels (A, B, C), each with 3 pick steps
\end{itemize}

\subsection{Inference Latency}

Per-frame processing times were characterized using YOLOv8n-pose benchmarks for the Jetson Nano 4\,GB with CUDA acceleration. The zone check and sequence tracking steps add sub-millisecond overhead on top of inference.
...
Effective FPS & ${\sim}$29.6\,fps \\
\bottomrule
\end{tabular}
\end{table}

The system sustains near-30\,fps throughput on the Jetson Nano. A deviation is detected and displayed within one frame period (${\sim}34$\,ms) of the wrist entering the wrong zone.

\subsection{Edge vs.\ Cloud Latency Comparison}

I compared edge feedback latency against a simulated cloud scenario modeling the full round-trip: frame capture $\rightarrow$ JPEG encode $\rightarrow$ MQTT publish $\rightarrow$ cloud inference $\rightarrow$ result return $\rightarrow$ alert display. Cloud network round-trip latency was set to 300\,ms, representative of a typical cloud data center connection; cloud inference was modeled at 20\,ms on server-class hardware. 100 samples were collected per scenario.
...
\toprule
\textbf{Scenario} & \textbf{Mean} & \textbf{Median} & \textbf{P95} & \textbf{Min} & \textbf{Max} \\
\midrule
Edge (Jetson Nano) & 33.8\,ms & 33.9\,ms & 43.9\,ms & 23.3\,ms & 45.9\,ms \\
Cloud (300\,ms RTT) & 324.1\,ms & 324.4\,ms & 331.5\,ms & 312.0\,ms & 333.4\,ms \\
\midrule
\textbf{Speedup} & \multicolumn{5}{c}{\textbf{9.6$\times$}} \\
\bottomrule
\end{tabular}
\end{table}

Edge inference is 9.6$\times$ faster than the simulated cloud path. Under more favorable network conditions (100\,ms RTT), cloud latency would still exceed 150\,ms---more than 4$\times$ the edge latency. Under poor conditions (500\,ms RTT), the gap widens to over 15$\times$. In all cases, only edge deployment keeps feedback latency within a single frame period (${\sim}33$\,ms), which is necessary for the alert to reach the operator while the erroneous pick is still in progress.
...
I measured the actual compressed size of 640$\times$480 video frames using 100 synthetic frames JPEG-encoded at quality 80, consistent with typical video analytics streaming settings. The edge approach transmits only a structured ${\sim}500$-byte JSON payload per completed assembly cycle.
...
\midrule
Avg frame size & --- & 43.7\,KB \\
Streaming rate @ 30\,fps & --- & 1.34\,MB/s \\
Data per cycle (${\sim}60$\,s) & ${\sim}500$\,B & ${\sim}81$\,MB \\
Data per hour (est.) & ${\sim}30$\,KB & ${\sim}4.8$\,GB \\
\textbf{Bandwidth reduction} & \multicolumn{2}{c}{\textbf{$>$161{,}000$\times$}} \\
\bottomrule
\end{tabular}
\end{table}

The edge approach reduces data transmission by over five orders of magnitude. At 1.34\,MB/s, a single 8-hour shift would generate approximately 38\,GB of video data per workstation; the edge system transmits under 250\,KB over the same period. This reduction also directly addresses operator privacy concerns, as raw video frames are never transmitted off the edge device.

\subsection{Detection Accuracy}
To evaluate the system's correctness, I measured detection accuracy over multiple monitored cycles using my prototype test setup with colored bins. I define two levels of accuracy:
...
% ================================================================
\section{Conclusion}
I presented an edge-based computer vision system for real-time process adherence monitoring on manufacturing assembly lines. The system runs YOLOv8n-pose estimation entirely on an NVIDIA Jetson Nano, tracking operator wrist movements to detect pick-sequence deviations and alert operators immediately with on-screen visual warnings. By performing all inference at the edge, the system achieves sub-frame feedback latency (${\sim}34$\,ms, a 9.6$\times$ improvement over a simulated cloud path), maintains operator privacy by never transmitting raw video, and reduces network bandwidth by over 161{,}000$\times$ compared to a cloud-based video streaming alternative. Structured cycle results are forwarded over MQTT to a PC-based web dashboard for historical analysis. The system is fully functional as a working prototype, supporting configurable zones, multiple trim levels, and both online (MQTT) and offline operation modes.
